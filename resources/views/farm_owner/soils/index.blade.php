<x-farm-owner-app-layout>
    ...
    <div class="bg-white rounded-xl border border-gray-200">
        <div class="p-5 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-[#2c3e2d]">Soil Health Scans</h3>
            <p class="text-sm text-gray-500 mt-1">Scans from your customers</p>
        </div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-[#f8f9f5] border-b border-gray-200">
                        ...
                        <th class="px-5 py-3 text-left text-xs font-semibold text-[#4f7942] uppercase tracking-wider">NPK (N-P-K)</th>
                        <th class="px-5 py-3 text-left text-xs font-semibold text-[#4f7942] uppercase tracking-wider">pH & Moisture</th>
                        <th class="px-5 py-3 text-left text-xs font-semibold text-[#4f7942] uppercase tracking-wider">Score</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($scans as $scan)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-5 py-4 text-sm font-medium text-gray-900">{{ $scan->customer ? $scan->customer->name : 'Unknown' }}</td>
                            ...
                            <td class="px-5 py-4 text-sm font-semibold">
                                @php
                                    ...
                                    $color = $score >= 70 ? 'bg-green-50 text-green-700' : ($score >= 40 ? 'bg-yellow-50 text-yellow-700' : 'bg-red-50 text-red-700');
                                @endphp
